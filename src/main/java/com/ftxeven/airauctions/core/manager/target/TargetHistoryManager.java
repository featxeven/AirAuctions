...
public final class TargetHistoryManager implements GuiManager.CustomGuiManager {
    ...
    public static final ThreadLocal<TargetHistoryHolder> CONSTRUCTION_CONTEXT = new ThreadLocal<>();

    public TargetHistoryManager(AirAuctions plugin) {
        this.plugin = plugin;
        loadDefinition();
    }

    public void loadDefinition() {
        File file = new File(plugin.getDataFolder(), "guis/target/target-history.yml");
        if (!file.exists()) plugin.saveResource("guis/target/target-history.yml", false);

        YamlConfiguration cfg = YamlConfiguration.loadConfiguration(file);
        ConfigurationSection section = cfg.getConfigurationSection("target-history");
        ...
        this.definition = new GuiDefinition(section.getString("title", "Target History"), section.getInt("rows", 6), items, section);
    }
    ...
    public Inventory build(Player viewer, Map<String, String> ph) {
        ...
        if (ph.containsKey("target_uuid")) {
            targetUuid = UUID.fromString(ph.get("target_uuid"));
        } else {
            targetUuid = plugin.database().records().uuidFromName(inputName);
        }
        ...
        List<AuctionHistory> history = plugin.database().history().getHistorySync(targetUuid, plugin.config().historyLimit());
        ...
        TargetHistoryHolder holder = new TargetHistoryHolder(realTargetName, targetUuid, page, totalPages, filterId, sortId, history);
        ...
        try {
            ...
            visualPh.put("target", realTargetName);
            ...
            for (AuctionHistory log : history) {
                boolean isSold = log.sellerUuid().equals(targetUuid);
                ...
                placedCount++;
                if (placedCount >= startIndex + itemsPerPage) break;
            }
            return inv;
        } finally {
    ...
    public void handleClick(InventoryClickEvent event, Player viewer) {
        ...
        if (!(event.getInventory().getHolder() instanceof TargetHistoryHolder holder) || event.getCurrentItem() == null) return;
    ...
    private void handleHistoryClick(InventoryClickEvent event, Player viewer, TargetHistoryHolder holder) {
        ...
        boolean isSold = targetLog.sellerUuid().equals(holder.targetUuid());
        GuiItem template = isSold ? itemSoldTemplate : itemBoughtTemplate;
    ...
    private void executeLogic(GuiItem item, Player viewer, TargetHistoryHolder holder) {
        ...
        switch (item.key()) {
            ...
            case "refresh" -> openMenu(viewer, holder.targetName(), holder.filter(), humanPage, holder.sort());
        }
    }

    private void openMenu(Player viewer, String targetName, String filter, int page, String sort) {
        plugin.core().gui().open("target_history", viewer, Map.of(
                "target", targetName,
                "filter", filter,
    ...
    @Override public boolean owns(Inventory inv) { return inv != null && inv.getHolder() instanceof TargetHistoryHolder; }
    ...
    public static final class TargetHistoryHolder implements PageableHolder {
        ...
        public TargetHistoryHolder(String targetName, UUID targetUuid, int page, int totalPages, String filter, String sort, List<AuctionHistory> history) {
