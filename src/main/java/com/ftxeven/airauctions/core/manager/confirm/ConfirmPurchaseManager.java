...
public final class ConfirmPurchaseManager implements GuiManager.CustomGuiManager {

    private final AirAuctions plugin;
    private final MiniMessage mm = MiniMessage.miniMessage();
    private YamlConfiguration config;

    public ConfirmPurchaseManager(AirAuctions plugin) {
        this.plugin = plugin;
        loadDefinition();
    }
    ...
    public Inventory build(Player viewer, Map<String, String> ph) {
        ...
        ConfigurationSection auctionItemSec = section.getConfigurationSection("auction-item");
        GuiItem auctionItemTemplate = auctionItemSec != null ? GuiItem.fromSection("auction-item", auctionItemSec) : null;
        ...
        GuiDefinition def = new GuiDefinition(section.getString("title", "Confirm Purchase"), section.getInt("rows", 3), items, section);

        ConfirmHolder holder = new ConfirmHolder(listing, ph, def, source);
        ...
        GuiSlotMapper.fill(plugin, inv, def, viewer, displayPh, Collections.singletonList(listing), auctionSlotSet, 0,
                auctionItemTemplate, null, Integer.MAX_VALUE);
    ...
    private String detectSource(Player viewer, Map<String, String> ph) {
        ...
        if (top.getHolder() instanceof ConfirmHolder prev) return prev.sourceGui();
    ...
    public void handleClick(InventoryClickEvent event, Player viewer) {
        ...
        if (!(event.getInventory().getHolder() instanceof ConfirmHolder holder) || event.getCurrentItem() == null) return;
    ...
    private void goBack(Player viewer, ConfirmHolder holder) {
    ...
    @Override public boolean owns(Inventory inv) { return inv != null && inv.getHolder() instanceof ConfirmHolder; }
    ...
    public static final class ConfirmHolder implements InventoryHolder {
        ...
        public ConfirmHolder(AuctionListing listing, Map<String, String> ph, GuiDefinition def, String sourceGui) {
        ...
        public PageableHolder toPageable() {
            return new PageableHolder() {
                ...
                @Override public Map<String, String> asContext() { return ConfirmHolder.this.asContext(); }
                @Override public @NotNull Inventory getInventory() { return ConfirmHolder.this.getInventory(); }
            };
        }
    }
}